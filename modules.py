...
def MLP(n_in, n_hid, n_out, do_prob=0.):
    ...
    x = Dense(n_hid, activation='elu', use_bias=True)(input)
    x = Dropout(do_prob)(x)
    x = Dense(n_out, activation='elu', use_bias=True)(x)

    org_shape = x.shape
    x = tf.reshape(x, shape=(org_shape[0]*org_shape[1], -1))
...
def CNN(n_in, n_hid, n_out, do_prob=0.):
    ...
    input = Input(shape=(n_in,))
    x = Conv1D(filters=n_hid, kernel_size=5, stride=1, activation='elu', padding='same', use_bias=True)(input)
    x = BatchNormalization()(x)
    x = Dropout(do_prob)(x)
    x = Conv1D(filters=n_hid, kernel_size=5, stride=1, activation='elu', padding='same', use_bias=True)(x)
    x = BatchNormalization()(x)
...
class MLPEncoder():
    ...
    def __init__(self, n_in, n_hid, n_out, do_prob=0., factor=True) -> None:
        self.factor = factor
        ...
        self.fc_out = Dense(n_out, input_shape=(n_hid,))

    def edge2node(self, x, rel_rec, rel_send):
        incoming = tf.matmul(rel_rec.t(), x)
        return incoming / incoming.size[1]

    def node2edge(self, x, rel_rec, rel_send):
        receivers = tf.matmul(rel_rec, x)
        senders = tf.matmul(rel_send, x)
        edges = tf.concat([senders, receivers], 2)
        return edges

    def __call__(self, inputs, rel_rec, rel_send):
        x = inputs
        x = self.mlp1()
        ...
        if self.factor:
            ...
            x = self.mlp3(x)
            x = self.node2edge(x, rel_rec, rel_send)
            x = tf.concat((x, x_skip), 2)  # Skip connection
            x = self.mlp4(x)            
        else:
            x = self.mlp3(x)
            x = tf.concat((x, x_skip), 2)  # Skip connection
...
class CNNEncoder():
    ...
    def node2edge_temporal(self, inputs, rel_rec, rel_send):
        ...
        x = tf.reshape(inputs, shape=(inputs.shape[0], inputs.shape[1], -1))

        receivers = tf.matmul(rel_rec, x)
        receivers = tf.reshape(receivers, shape=(
            inputs.shape[0]*inputs.shape[1], \
            inputs.shape[2], inputs.shape[3]))
        receivers = tf.transpose(receivers, perm=(2,1))

        senders = tf.matmul((rel_send, x))
        senders = tf.reshape(senders, shape=(
            inputs.shape[0]*inputs.shape[1], \
            inputs.shape[2], inputs.shape[3]))
        senders = tf.transpose(senders, perm=(2,1))

        edges = tf.concat([senders, receivers], axis=1)
        return edges

    def edge2node(self, x, rel_rec, rel_send):
        incoming = tf.matmul(tf.transpose(rel_rec), x)
        return incoming / incoming.shape[1]

    def node2edge(self, x, rel_rec, rel_send):
        receivers = tf.matmul(rel_rec, x)
        senders = tf.matmul(rel_send, x)
        edges = tf.concat([senders, receivers], axis=2)
        return edges
    ...
    def __call__(self, inputs, rel_rec, rel_send) :
        ...
        x = tf.reshape(x, shape=(
            inputs.shape[0], (inputs.shape[1]-1)*inputs.shape[1], -1))
        x = self.mlp1(x)
        x_skip = x        
        ...
        if self.factor:
            ...
            x = self.node2edge(x, rel_rec, rel_send)
            x = tf.concat((x, x_skip), axis=2)  # Skip connection
            x = self.mlp3(x)

        return self.fc_out(x)

class SimulationDecoder():
...
class MLPDecoder():
    ...
    def __init__(self, n_in_node, edge_types, msg_hid, msg_out, n_hid,
        ...
        self.msg_fc1 = Sequential(
            [Dense(2 * n_in_node, msg_hid) for _ in range(edge_types)])
        ...
        self.out_fc1 = Dense(n_hid) # input_dim = n_in_node + msg_out
        self.out_fc2 = Dense(n_hid) # input_dim = n_hid
        self.out_fc3 = Dense(n_in_node) # input_dim = n_hid
    ...
    def single_step_forward(self, single_timestep_inputs, rel_rec, rel_send,
        ...
        # Node2edge
        receivers = tf.matmul(rel_rec, single_timestep_inputs)
        senders = tf.matmul(rel_send, single_timestep_inputs)
        pre_msg = tf.concat([senders, receivers], axis=-1)

        all_msgs = tf.zeros(pre_msg.size(0), pre_msg.size(1),
        ...
        for i in range(start_idx, len(self.msg_fc2)):
            msg = relu(self.msg_fc1[i](pre_msg))
            msg = F.dropout(msg, p=self.dropout_prob)
            msg = relu(self.msg_fc2[i](msg))
            msg = msg * single_timestep_rel_type[:, :, :, i:i + 1]
            all_msgs += msg

        # Aggregate all msgs to receiver
        agg_msgs = tf.transpose(all_msgs, perm=(-2, -1))
        agg_msgs = tf.matmul(agg_msgs, rel_rec)
        agg_msgs = tf.transpose(all_msgs, perm=(-2, -1))

        # Skip connection
        aug_inputs = tf.concat([single_timestep_inputs, agg_msgs], axis=-1)

        # Output MLP
        pred = Dropout(self.dropout_prob)(relu(self.out_fc1(aug_inputs))
